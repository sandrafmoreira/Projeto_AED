...
#       Chama as classes Nav_Bar e Homepage, respetivamente
        self.Nav_Bar(self.nav_bar, self.homepage, self.tl)
        self.Homepage(self.username, self.homepage)
    ...
    class Nav_Bar:
        ...
        """
        def __init__(self, nav_bar, homepage, tl):
...
#           Icone da homepage (FONTE - SITE FLATICON)
            icon = Image.open('..\\Projeto_AED\\images\\icons\\homepage_icon.png').resize((50,50))
...
#           Icone do perfil (FONTE - SITE FLATICON)
            icon2 = Image.open('..\\Projeto_AED\\images\\icons\\profile_icon.png').resize((50,50))
            icon2 = ImageTk.PhotoImage(icon2)
            self.profile_icon = Button(self.nav_bar, image = icon2, bd = 0, bg = '#333333', command = self.profile_click)
...
#           Janela com várias estatísticas do utilizador
            icon3 = Image.open('..\\Projeto_AED\\images\\icons\\dashboard_icon.png').resize((50,50))
...
#           Icone de notificações (FONTE - SITE FLATICON)
            icon4 = Image.open('..\\Projeto_AED\\images\\icons\\bell_icon.png').resize((50,50))
...
#           search = Entry(nav_bar, bg = '#E0E0E0', bd = 0, font =('Roboto', 14), width = 15).place(x = 220, y = 15)
    ...
    class Homepage:
        def __init__(self, username, homepage):
            """
...
#           NOTA: FAZER RETURN ADMIN!!!!
            if self.username == 'nuno' or self.username == 'sandra' or self.username == 'ken':
...
#           Label
            self.popular_posts_lbl = Label(self.homepage, text = 'Most Popular Posts',font = ('Roboto', 20), bg = 'lightgrey').place( x = 30, y = 130)

#           Botão para adicionar um post
            self.add_post_btn = Button(self.homepage, text = '+  Add', width = 12, height = 1, bg = '#28942a', fg = 'white', font = ('Roboto', 20), bd = 0, command = self.show_add_content_frame).place(x = 800, y = 130)
...
#           Aparece os botões de Fazer um Post e Criar um Album
        ...
        def show_add_content_frame(self):
            ...
            if self.add_content_frame.winfo_ismapped(): #Se o frame estiver na janela
                ...
                self.add_content_frame.place(x = 800, y = 180)
